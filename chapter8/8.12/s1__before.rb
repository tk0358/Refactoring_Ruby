...
class MountainBikeTest < Minitest::Test
  def setup
    @rigid_bike = MountainBike.new(:type_code => :rigid, :tire_width => 2.5)
    @front_suspension_bike = MountainBike.new(:type_code => :front_suspension, :tire_width => 2,
      :front_fork_travel => 3)
    @full_suspension_bike = MountainBike.new(:type_code => :full_suspension, :tire_width => 2,
